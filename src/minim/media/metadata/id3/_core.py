    ...
    * :code:`__len__` – Return the number of fields.
    ...
    def _from_stream_2_4(
        ...
        while offset < tag_end:
            ...
            obj._add_frames(frame, strict=strict)
    ...
    def comment(
        self,
        value: str | ID3v2COMMFrame | OrderedCollection[ID3v2COMMFrame],
        /,
    ) -> None:
    ...
    def lyrics(self) -> list[str] | None:
        """
        :bdg-primary:`get` :bdg-secondary:`set`
        :code:`ULT`/:code:`USLT` – Lyrics or transcription.
        """
        if frames := self._class_index.get(ID3v2Frame._get_class(b"USLT")):
            return [frame._lyrics for frame in frames]

    @lyrics.setter
    def lyrics(
        self,
        value: str | ID3v2USLTFrame | OrderedCollection[ID3v2USLTFrame],
        /,
    ) -> None:
        frame_cls = ID3v2Frame._get_class(b"USLT")
        match value:
            case list() | tuple():
                for idx, val in enumerate(value):
    ...
    def _add_frames(
        self,
        frames: ID3v2Frame | OrderedCollection[ID3v2Frame],
        /,
        *,
        ...
        """
        Add ID3v2 frames to the data structures in an :class:`ID3v2`
        object.
        ...
        strict : bool; keyword-only; default: :code:`True`
            ...
            specifications.
        """
        for frame in [frames] if isinstance(frames, ID3v2Frame) else frames:
            match frame:
                case EncryptedID3v2Frame():
                    if (
                        strict
                        and not frame._allow_multiple
                        and self._class_index.get(frame._class)
                    ):
                        raise ValueError(
                ...
                case UnknownID3v2Frame():
                    ...
                    self._unknown_index[frame._frame_id].append(frame)
                case _:
                    if (frame_cls := type(frame))._allow_multiple:
                        if (
                            strict
                            and (
                                existing_frame_keys := self._key_index.get(
                                    frame_cls
                                )
                            )
    ...
    def _set_known_frames(
        ...
        if isinstance(frames, ID3v2Frame):
            ...
            if frame_cls is EncryptedID3v2Frame:
                frame_cls = frames._class
            frames = [frames]
        else:
            frame_cls = type(frames[0])
            if frame_cls is EncryptedID3v2Frame:
                frame_cls = frames[0]._class
        ...
        for frame in frames:
            (
                encrypted_frames
                if isinstance(frame, EncryptedID3v2Frame)
                else unencrypted_frames
            ).append(frame)
        self._class_index[frame_cls] = unencrypted_frames
        self._class_index[EncryptedID3v2Frame] = [
            frame
            for frame in self._class_index[EncryptedID3v2Frame]
            if frame._class is not frame_cls
        ]
        self._class_index[EncryptedID3v2Frame].append(encrypted_frames)
    ...
    def append(
        ...
        *,
        ...
        for idx, frame in enumerate(frames):
            ...
            self._add_frames(frame, strict=strict)
    ...
    def get(
        ...
        if not (
            ...
            and all(
                isinstance(frame_type, bytes | ID3v2Frame)
                for frame_type in frame_types
            )
        ):
            raise TypeError(
                "`frame_types` must be one or more bytes and/or "
                "ID3v2Frame objects."
            )

        return {frame_type: self.get(frame_type) for frame_type in frame_types}
    ...
    def set(
        ...
        for idx, frame in enumerate(
            [frames] if isinstance(frames, ID3v2Frame) else frames
        ):
            validate_type(f"frames[{idx}]", frame, ID3v2Frame)
            frame_cls = type(frame)
            if frame_cls is UnknownID3v2Frame:
                raise ValueError(
                    f"Unknown {frame._frame_id} frames cannot be set; "
                    "they can only be appended."
                )

            if frame_cls is EncryptedID3v2Frame:
                frame_cls = frame._class
                if frame_cls is UnknownID3v2Frame:
                    raise ValueError(
                        f"Unknown {frame._frame_id} frames cannot be "
                        "set; they can only be appended."
                    )

            if not frame._allow_multiple and frame_cls in frames_by_type:
